...
function getDTDistribution_GroupByProandDec(system, project, department ,station,lastday, currentDay) {
    getDataWithArray("/Dashboard/GetDTDistribution", { comefrom: system, departmentlist: department, projectlist: project, stationlist: station, currentDay: currentDay, lastday: lastday })
        .then(res => {
            if (res.length == 0) {
            ...
            for (var i = 0; i < res.length; i++) {
                ...
                dataArray.data.find(e => e.name == res[i].department).value[dataArray.axis.findIndex(e => e.name == res[i].project)] = res[i].value;
            }
            return dataArray;
        })
        .then(res => {
